...
To do so, they must have a watchful eye. 
If the sales manager wants something done and tries to persuade one of the developers at the coffee machine, they are kindly redirected to the product owner (see below) to get it on the back log. 
If some of the team members lack some necessary skills, the **scrum master** will make a plan with them on how to acquire them. 
Also, they will be the organiser and facilitator of the different Scrum meetings (called ceremonies). 

The **product owner** is responsible for what the product looks like. 
He monitors the customer's needs and desires, so one of their key responsibilities is stakeholder management. 
He translates feature requests and improvement plans into user stories, thereby maintaining the product log and ordering the stories by importance.
...
At the beginning of the sprint there is the **sprint planning**, in which the stories are scoped and the definition of "completed" is determined. 
...
It does not work with fixed time units, such as the Scrum sprints, but it aims instead to achieve a continuous flow. 
...
Central is the Kanban board, which can be physical or virtual, and has at least the following columns: *to do*, *doing*, and *done*. This can be tailored to the wishes and needs of the team. 
...
Effective Kanban teams have short cycle times, meaning that they are able to complete tasks quickly. 
The cycle time enables the team to give time estimates for when work will be completed with confidence. 
...
The highly structured Scrum and the lightweight Kanban are two workflows that could make a team work closer to the Agile principles. 
...
In Scrum the team commits to the stories it selected to complete in this sprint and the building has to be on fire before it will take on work that is not in the sprint. Kanban only prescribes a limit to the number of stories that are in progress, encouraging to finish tasks that have already been started before taking on something new. 
...
Both methodologies are applied with great success and it's important to keep in mind that they are means to an end, not religions. 
The Agile values and principles should be the primary guideline and when selecting one of the workflows you do so because it is the best way to work for the given situation. 
