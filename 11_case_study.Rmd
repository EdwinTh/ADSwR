...
We had to provide him with the information required to make such decision.
A *Shiny* dashboard appeared to be the way to go.
In this dashboard who could see the basic model performence, reflected in an agreed upon statistic.
...
My background is in statistics, not software engineering, I could not tell what it requires to expose a model to the millions of visitors of our website.
...
Up until this point I am not sure if it is possible to create an R API that is up to task.
Sometimes it argued by python evangelicals that you should only use python because you can do everything in one language.
