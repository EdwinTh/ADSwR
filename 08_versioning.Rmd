...
Whether versioned or not, you might be inclined to leave old versions of a product, such as a function to call a model or to create a report, in place.
...
They check each other's work by doing code reviews.
The version control system can even be configured such that a branch can only be merged to master when the changes are approved by at least one or two colleagues.
