...
Many projects following this methodology failed because of the long duration of these projects. 
...
There might be unclarities on what to predict or which data sources are in scope, due to lack of communication between stakeholders, business people and data scientists. 
...
This principle is a revolutionary break from Waterfall, you should not wait with releasing software until it's perfect, instead get it out in the open when it is just good enough. 
A common term used for this is the MVP (*Minimal Viable Product*). 
After the MVP is released it is closely monitored for how users are interacting with it and where the biggest room for improvement is. 
...
As the projects drags on without any output they are likely to loose interest and confidence the project will end well. Eventually they might pull the plug or put it on hold before anything reaches production. 
If they can interact with the results soon, even if it is imperfect, it will give a totally different dynamic.
* *You will fail fast*. 
There is a wide array of reasons a data science project might fail, such as: the problem appears not be translatable into a model in the first place, the data is not of the quality needed, there appears not be enough historical data, or the necassary relationships simply don't exist. 
...
What an MVM looks like is project-dependent of course, but in many cases it would probably make sense to define it as a regular statistical measure. 
...
This principle comes naturally to data science, since the outcome of a project typically depends on the relationships discovered in data. 
The Waterfall approach in which every step of the project is planned would appear hideous to even the biggest lover of process in the data science context. 
...
This creates momentum and excitement for customers, stakeholders, your teammates and yourself. 
...
Instead of meticuously describing how the job should be done, just set the goals of the projects and leave it up to the team how these goals should be attained. 
Data scientists typically already enjoy this type of freedom for the sheer reason that stakeholders often don't really understand how the products are built. 
...
If you think your approach is better, take the time to explain why you think a different approach is better (in layman terms of course). 
...
The developer goes to work, and having ample time before the deadline there are many things that can be explored. 
...
They promote a workflow of taking it easy at first, stressing out when the deadline comes near and exhaustion after it. Instead set small goals that are attainable in a short timespan, update project if the results are favorable and set a new small goal. 
...
Cleaning of the training data, splitting into train and validation sets, running algorithms that produce the models, doing research on relationships in the data and many more steps are for the data scientist's eyes only. 
...
Always consider future you as a separate person who you respect very much and you want to help to do their job as well as possible. 
...
A machine learning project's goal is often straightforward, predict *y* as best you can such that some business goal can be achieved. 
Other than software development there is not much to choose for which features should and should not be included in the final product (features as in characteristics, not as in predictors). 
The options of how to arrive at predicting *y*, however, are abundant. 
...
We can roughly estimate what the amount of work would be to explore both options. It is, however, very hard to predict what the amount of value is that the new part will add. 
...
Every development process has its inefficiencies, whether they are unclear goals, poor communication or not having the right priorities. Having to reflect on the process forces you to look critically at all aspects of the project. 
...
Even when you are the only data scientist or even the only developer of the team, you should also address your technical issues here. 
