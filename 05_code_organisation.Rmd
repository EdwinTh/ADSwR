...
It does not work for large scale projects in which the data prodcut should be reliable and stable, because this way of working results in low reproducibility and inflexibility towards new situations. 
...
It gets unclear which reported results are obtained on which data set and if this was before or after some modifications to the code. 
...
These operations are then chained together to create the entire product. 
If your product has somekind of flow in it, such as the scoring of a machine learning model of turning last month's data into a research report, you can apply the functions in a pipeline.
...
Doing Agile Data Science with R relies on proper use and understanding of the R package structure. Hadley Wickham wrote a comprehensive book on developing software in R packages. It is freely available online, [here](http://r-pkgs.had.co.nz/). You should read it! <!--- soon to be updated to https://r-pkgs.org/ --->
