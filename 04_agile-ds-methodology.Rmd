...
Now we have interpreted the Twelve Agile Principles in the data science setting, we explore what an Agile data science workflow might look like. 
...
(Note that it is the tasks that are typically linear, the entire process of building a product is iterative by nature).
In contrast are data science tasks are circular, at the start of a task it is uncertain what the outcome will be. 
...
In order to test hypotheses quickly, exploration scripts can be interactive analyses without software requirements or even being reproducible in later stages. They should quickly give an indication if the tested hypothesis could improve the product.
...
If you try to define a user story for a machine learning model it will always go "As the <user of the model> I want to have the best possible model, such that <application of the model> is achieved best". 
...
From its definition it is clear what part of the code can be touched by it and which not.
...
This will make sure that the tasks that are currently in *doing* gets completed first, and that after each completion there is a moment where it can be decided what is the most urgent change to the pipeline or the most promising hypothesis to explore next. 
As a rule of thumb, never work on more tasks simultaneously than that there are data scientists on the team. 
