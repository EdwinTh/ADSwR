...
Therefore, this chapter should be considered as nothing more than an exploration, a bunch of thoughts. 
If some of it does not work for you for whatever reason, by all means find a better way.

## Linear and Circular Tasks

The tasks (or stories) in Agile software development are what I call linear tasks. 
They come from prodcut feature requets by stakeholders, collected by the product owner. 
...
Building a product upon relationships in data results in what I call circular tasks. 
The starting position is the latest version of the product. 
If one does not yet have a good idea how to further improve it we would typically do an explorative analysis. 
From this a hypothesis can be generated of how the product might be improved, which is subsequently tested. 
Next we can evaluate if we want to update the product based on the hypothesis tested. 
Both when updated and when not updated we start a new cycle after the task is completed.
...
We simply cannot guarantee that a statistical model or a machine learning algorithm will be improved in two weeks time, because we don't know if the hypotheses we are going to test will give us anything. 
Moreover, during the completion of a task we can generate new hypotheses that seem more relevant than the one we originally had scheduled to do after this task. 
...
In the remainder of this text we assume projects to at least partially build on relationships that are unknown at the start of the project, and thus containing circular tasks. 
...
The data product is the software that gets productionised. 
...
Or it can be the querying of different databases, joining the different data, calculating statistics and building plots, wrapping everything in a Markdown report that gets sent to different stakeholders. 
...
Exploratory research will greatly benefit from having a data product, because intermediate results on which to further explore quickly are immediately available. 
In order to test hypotheses quickly, exploration scripts can be interactive analyses without software requirements or maybe not even reproducible in later stages. 
They should quickly give an indication if the tested hypothesis could improve the current product.
...
In data science, especially machine learning or statistical modelling, the final product is static, such as a model or a bunch of predictions. 
...
On a task level, as a rule of thumb, user stories make sense for linear tasks, but not for circular ones. 
...
Kanban on the other hand gives us the flexibility to change the next task we take upon ourselves. 
...
It appears that the linear tasks of software design are a match with building the data product, whereas the circular tasks are applicable to the exploratory research. 
...
As a rule of thumb, never work on more tasks simultaneously than there are data science members on the team. 
...
<!--- the previous sentence is a little awkward. Maybe this: 
", which directly indicate the requirements for the completion of the task --->
...
In fact, the average time of task completion is Kanban's key metric of effectiveness. 
...
We like to have just one more look from this other angle, or maybe this new fancy algorithm that you have been anxious to test for a while will give a major boost. 
...
This could lead to stubborness, however, unwilling to give up what was thought to be the way to get a major improvement. 
...
If he found an alleyway that is still worthwhile exploring a new task should be put in the backlog, instead of preservering it in the current task. 
...
You should discuss the tasks you put on the Kanban board with the product owner, even when they're technical. 
...
You have to find out for yourself as a team how you reflect on this is, but it might make sense to use the biweekly rhythm of Scrum. 
You might combine it with scoping tasks that were put on the board recently and reprioritizing your work. 
